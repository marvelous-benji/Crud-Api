...
def test_create_template(client,token):
    data = {"template_name": "temp_test", "subject": "testing", "body": "testing the api"}
    headers = {"Content-Type": "application/json", "Authorization":f"Bearer {token[0]}"}
    response = client.post(
        "/api/v1/template", headers=headers, data=json.dumps(data)
    )
...
def test_get_templates(client,token,default_template):
    ...
    headers = {"Content-Type": "application/json", "Authorization":f"Bearer {token[0]}"}
    response = client.get(
        "/api/v1/template", headers=headers)
...
def test_get_template(client,token, default_template):
    ...
    headers = {"Content-Type": "application/json", "Authorization":f"Bearer {token[0]}"}
    response = client.get(
        f"/api/v1/template/{temp_id}", headers=headers
    )
    assert response.status_code == 200
    response = json.loads(response.get_data(as_text=True))
    assert "success" == response["status"]
...
def test_put_template(client,token, default_template):
    temp_id = default_template[0]
    data = {"template_name": "temporary_test", "subject": "testing101", "body": "testing the api"}
    headers = {"Content-Type": "application/json", "Authorization":f"Bearer {token[0]}"}
    response = client.put(
